...
public class Board {
            ...
            // corresponding list
            {
                switch (boardArray[to]) {
                    ...
                    case B_QUEEN:
                        ...
                        break;
                    case W_KING:
                        Notification.getInstance().getLogger().error(getFen());
                        Notification.getInstance().getLogger().error("White king was captured");
                        break;
                    case B_KING:
                        Notification.getInstance().getLogger().error(getFen());
                        Notification.getInstance().getLogger().error("White king was captured");
    ...
    public final boolean validateHashMove(int move) {
        ...
        if (possiblePieces == ATTACK_NONE) {
            return false;
        }

        if (toMove == WHITE_TO_MOVE) {
            if (piece == W_QUEEN && !(possiblePieces == ATTACK_KQBbP || possiblePieces == ATTACK_KQBwP || possiblePieces == ATTACK_KQR || possiblePieces == ATTACK_QB || possiblePieces == ATTACK_QR)) {
                return false;
            }
            if (piece == W_ROOK && !(possiblePieces == ATTACK_KQR || possiblePieces == ATTACK_QR)) {
                return false;
            }
            if (piece == W_BISHOP && !(possiblePieces == ATTACK_KQBbP || possiblePieces == ATTACK_KQBwP || possiblePieces == ATTACK_QB)) {
                return false;
            }
            if (piece == W_KNIGHT && !(possiblePieces == ATTACK_N)) {
                return false;
            }
            ...
            if (piece == W_PAWN) {
                if (type == EN_PASSANT && to == enPassant) {
                    return true;
                }
                if (capture == 0) {
                    ...
                    if (possiblePieces != ATTACK_KQBwP) {
                        return false;
                    }
        ...
        {
            if (piece == B_QUEEN && !(possiblePieces == ATTACK_KQBbP || possiblePieces == ATTACK_KQBwP || possiblePieces == ATTACK_KQR || possiblePieces == ATTACK_QB || possiblePieces == ATTACK_QR)) {
                return false;
            }
            if (piece == B_ROOK && !(possiblePieces == ATTACK_KQR || possiblePieces == ATTACK_QR)) {
                return false;
            }
            if (piece == B_BISHOP && !(possiblePieces == ATTACK_KQBbP || possiblePieces == ATTACK_KQBwP || possiblePieces == ATTACK_QB)) {
                return false;
            }
            if (piece == B_KNIGHT && !(possiblePieces == ATTACK_N)) {
                return false;
            }
            ...
            if (piece == B_PAWN) {
                ...
                if (capture == 0) {
                    ...
                    if (possiblePieces != ATTACK_KQBbP) {
                        return false;
                    }
                    return (((from - 15) == to || (from - 17) == to) && boardArray[to] == capture);
                }
    ...
    private final int getInterveneMoves(Move[] moves, int attacked, int side, int startIndex) {
        ...
        if (side == WHITE) {
            ...
            for (int i = 0; i < w_pawns.count; i++) {
                ...
                if (file(from) != file(attacked)) {
                    continue; // Can't intervene if not on the same file
                }
    ...
    public boolean isInCheck() {
        int king_square;
        if (toMove == WHITE_TO_MOVE) {
            king_square = w_king.pieces[0];
        } else {
            king_square = b_king.pieces[0];
        }
