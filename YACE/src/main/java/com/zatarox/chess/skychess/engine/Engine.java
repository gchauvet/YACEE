...
import com.zatarox.chess.skychess.board.SEE;
import com.zatarox.chess.skychess.MainApp;
import com.zatarox.chess.skychess.Settings;
...
public class Engine {
    ...
    public static LineEval search(Board board, int depth, int timeLeft, int increment, int movetime, boolean isPonder) throws IOException {
        ...
        if (board.gen_allLegalMoves(searchMoves[0], 0) == 0) {
            return finalEval;
        }
        ...
        stopSearch = false;
        if (movetime == 0) {
            timeForThisMove = calculateTime(board, timeLeft, increment); // Calculate the time for this move
        } else {
    ...
    public static Move alphaBetaRoot(Board board, int depth, int alpha, int beta, boolean allowNull, int ply) throws IOException {
        ...
        if (searchedMoves == 0) {
            if (board.isInCheck()) {
                bestMove.score = (MATE_VALUE + ply);
            } else {
                bestMove.score = DRAW_VALUE;
            }
        }
    ...
    public static int alphaBeta(Board board, int depth, int alpha, int beta, boolean allowNull, int ply) throws IOException {
        ...
        if (stopSearch) {
            return 0; // Stop the search if it's been detected
        }
        ...
        while (generationState < GEN_END) {
                ...
                case GEN_CAPS:
                    ...
                    for (int i = 0; i < currentMovesCount; i++) {
                        ...
                        if (tempMove == hashMove) {
                            ...
                            searchMoves[ply][i].score = SEE.see(board, tempMove); // The move is not a duplicate so give it a see value
                        }
                    }
            ...
            for (int i = startIndex; i < currentMovesCount; i++) {
                ...
                if (Move.pieceMoving(searchMoves[ply][i].move) == W_PAWN && Board.rank(Move.toIndex(searchMoves[ply][i].move)) == 6) {
                    threat = true;
                } else if (Move.pieceMoving(searchMoves[ply][i].move) == B_PAWN && Board.rank(Move.toIndex(searchMoves[ply][i].move)) == 1) {
                    threat = true;
                }
                ...
                if (searchedMoves >= 1 && fprune && !threat && !board.isInCheck()) {
                    ...
                    if ((materialEval + gain + fmargin) <= alpha) {
                        if (materialEval + gain > bestEval) {
                            bestEval = materialEval + gain;
                        }
                        continue;
                    }
                }
                ...
                if (eval > bestEval) {
                    ...
                    if (eval >= beta) {
                        ...
                        if (!stopSearch) {
                            Settings.getInstance().getTranspositionTable().record(board.zobristKey, depth / PLY, HASH_BETA, eval, searchMoves[ply][i].move);
                        }
        ...
        if (!stopSearch) {
            Settings.getInstance().getTranspositionTable().record(board.zobristKey, depth / PLY, eval_type, bestEval, bestMove);
        }
    ...
    private static int calculateTime(Board board, int timeLeft, int increment) {
        ...
        if (timeForThisMove >= timeLeft) {
            timeForThisMove = timeLeft - 500; // If the increment puts us above the total time left use the timeleft - 0.5 seconds
        }
    ...
    public static void setStopSearch(boolean stopSearch) {
        Engine.stopSearch = stopSearch;
    }

    public static void setPonder(boolean ponder) {
        Engine.ponder = ponder;
    }
    ...
    private static String receiveThinking(long time, LineEval finalEval) {
        ...
        int nps;
        if ((splitTime / 1000) < 1) {
            nps = totalNodesSearched;
        } else {
            Double decimalTime = totalNodesSearched / (splitTime / 1000D);
