...
public class Evaluation {
    ...
    public static final int evaluate(Board board, boolean useHash) {
        ...
        for (int i = 0; i < board.w_pawns.count; i++) {
            ...
            if (rank == 1) {
                wPawnOnSecond = true; // Atleast one pawn on the second rank so placing a rook/queen there might be worth it 
            }
        }
        ...
        for (int i = 0; i < board.b_pawns.count; i++) {
            ...
            if (rank == 6) {
                bPawnOnSeventh = true; // Atleast one pawn on the seventh rank so placing a rook/queen there might be worth it
            }
        }
    ...
    public static final void w_kingDefense(Board board) {
        ...
        if (king_file < 3) {
            ...
            if ((w_pawnPos[2] & 0xFFFF) == 2 && board.boardArray[B2] == W_BISHOP) {
                totalDefense += 20;
            }
        } // King on kingside
        else if (king_file > 4) {
            ...
            // points for proper fianchetto
            if ((w_pawnPos[7] & 0xFFFF) == 2 && board.boardArray[G2] == W_BISHOP) {
                totalDefense += 20;
            }
        }
        ...
        if ((king_index == E1 && board.boardArray[F2] != W_PAWN)
                || (king_index == D1 && board.boardArray[C2] != W_PAWN)) {
            totalDefense -= 10;
        }
    ...
    public static final void b_kingDefense(Board board) {
        ...
        if (king_file < 3) {
            ...
            // points for proper fianchetto
            if ((b_pawnPos[2] & 0xFFFF) == 5 && board.boardArray[B7] == B_BISHOP) {
                totalDefense += 20;
            }
        } // King on kingside
        else if (king_file > 4) {
            ...
            // points for proper fianchetto
            if ((b_pawnPos[7] & 0xFFFF) == 5 && board.boardArray[G7] == B_BISHOP) {
                totalDefense += 20;
            }
        }
        // If the king is still on the original square, the F2 square is weak so
        ...
        if ((king_index == E8 && board.boardArray[F7] != B_PAWN)
                || (king_index == D8 && board.boardArray[C7] != B_PAWN)) {
            totalDefense -= 10;
        }
    ...
    public static final void w_kingAttacked(Board board) {
        ...
        if (((attackedIndex) & 0x88) == 0 && BB[attackedIndex] != 0) {
            ...
            if (WB[attackedIndex] == 129) {
                attackedCount++; // 129 translates to 10000001 in binary form, meaning only one attacker, and that attacker is the king 
            }
        }
        attackedIndex = kingIndex - 1;
        if (((attackedIndex) & 0x88) == 0 && BB[attackedIndex] != 0) {
        ...
        if (((attackedIndex) & 0x88) == 0 && BB[attackedIndex] != 0) {
            ...
            if (board.boardArray[attackedIndex] <= 0) {
                attackedCount++; // Empty square or enemy piece just in front of the king
            }
            if (WB[attackedIndex] == 129) {
                attackedCount++; // 129 translates to 10000001 in binary form, meaning only one attacker, and that attacker is the king 
            }
    ...
    public static final Value evaluatePassers(Board board) {
        ...
        while (whitePassers != 0) {
            ...
            if (gamePhase == PHASE_PAWN_ENDING) {
                ...
                if (Board.file(board.w_king.pieces[0]) == file && Board.rank(board.w_king.pieces[0]) > rank) {
                    promDist++;
                }
        ...
        while (blackPassers != 0) {
            ...
            if (gamePhase == PHASE_PAWN_ENDING) {
                ...
                if (Board.file(board.b_king.pieces[0]) == file && Board.rank(board.b_king.pieces[0]) < rank) {
                    promDist++;
                }
                if (rank == 6) {
                    promDist--;
    ...
    public static final int w_trapped(Board board) {
        ...
        if (board.boardArray[A7] == W_BISHOP && board.boardArray[B6] == B_PAWN) {
            ...
            if (board.boardArray[C7] == B_PAWN) {
                trapped_eval -= 50; // Even more if the trapping pawn is supported
            }
        }
        ...
        if (board.boardArray[D2] == W_PAWN && board.boardArray[D3] != EMPTY_SQUARE) {
            ...
            if (board.boardArray[C1] == W_BISHOP) {
                trapped_eval -= 30; // Even more if there is still a bishop on c1
            }
        }
        if (board.boardArray[E2] == W_PAWN && board.boardArray[E3] != EMPTY_SQUARE) {
            trapped_eval -= 20;
            if (board.boardArray[F1] == W_BISHOP) {
                trapped_eval -= 30; // Even more if there is still a bishop on f1
            }
        }
        return trapped_eval;
    }
